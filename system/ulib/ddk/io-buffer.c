...
zx_status_t io_buffer_physmap_range(io_buffer_t* buffer, zx_off_t offset,
    ...
    if (io_default_bti == ZX_HANDLE_INVALID) {
        ...
        const size_t num_pin_entries = ROUNDUP(length, io_default_bti_min_contig) /
                io_default_bti_min_contig;
        ZX_DEBUG_ASSERT(phys_count >= num_pin_entries);
        zx_paddr_t* pin_entries = physmap + (phys_count - num_pin_entries);
        zx_status_t status = zx_bti_pin(io_default_bti, buffer->vmo_handle, pin_offset,
                                        pin_length,
                                        ZX_VM_FLAG_PERM_READ | ZX_VM_FLAG_PERM_WRITE,
                                        pin_entries, num_pin_entries, &actual_entries);
        if (status != ZX_OK) {
            return status;
        }
        if (actual_entries != phys_count) {
            // Special case contiguous mappings which return single entries
            if (actual_entries == 1) {
                physmap[0] = pin_entries[0];
                for (size_t i = 1; i < phys_count; ++i) {
                    physmap[i] = physmap[i - 1] + PAGE_SIZE;
                }
            } else {
                size_t physmap_idx = 0;
                // Note that pin_entries and physmap are aliased (in particular,
                // pin_entries is the tail of physmap).  Since each entry in
                // pin_entries is at least a page, filling physmap left-to-right
                // will avoid trampling on the pin_entries data until we're done
                // with it.
                for (size_t i = 0; i < num_pin_entries; ++i) {
                    for (uint64_t offset = 0;
                         offset < io_default_bti_min_contig && physmap_idx < phys_count;
                         offset += PAGE_SIZE) {
                        physmap[physmap_idx++] = pin_entries[i] + offset;
                    }
                }
            }
        }
