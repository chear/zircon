...
public:
    ...
    zx_status_t pin(const vmo& vmo, uint64_t offset, uint64_t size, uint32_t perms,
                    zx_paddr_t* addrs, size_t addrs_count, size_t* actual_addrs_count) const {
        return zx_bti_pin(get(), vmo.get(), offset, size, perms, addrs, addrs_count,
                          actual_addrs_count);
    }
